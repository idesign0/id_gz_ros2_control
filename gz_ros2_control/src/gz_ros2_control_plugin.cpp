      ...
      case sdf::JointType::UNIVERSAL:
        {
          RCLCPP_WARN_STREAM(
            node_->get_logger(),
            "[gz_ros2_control] Joint [" << jointName << "] (Entity=" << jointEntity << ") is of unsupported type."
      ...
      default:
        {
          RCLCPP_WARN_STREAM(
            node_->get_logger(),
            "[gz_ros2_control] Joint [" << jointName << "] (Entity=" << jointEntity << ") is of unknown type."
          );
          continue;
        }
    }
  ...
  sim::EventManager &)
{
  ...
  if (!model.Valid(_ecm)) {
    RCLCPP_ERROR_STREAM(
      logger,
      "[Gazebo ROS 2 Control] Failed to initialize because [" << model.Name(_ecm) << "] (Entity=" << _entity << ") is not a model."
